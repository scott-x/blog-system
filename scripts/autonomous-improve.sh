...
security_check() {
    ...
    local patterns=(
        ...
        # JWT tokens (short, often a signal)
    ...
    for f in $files; do
        # 只检查代码和配置文件
        case "$f" in
            *.go|*.ts|*.tsx|*.js|*.jsx|*.yaml|*.yml|*.json|*.env|*.md|*.txt)
                ;;
            *)
                continue
                ;;
        ...
        if [ -f "$f" ]; then
            for pattern in "${patterns[@]}"; do
                # 用 grep 检查（只显示文件名，不显示匹配内容）
                if grep -Eq "$pattern" "$f" 2>/dev/null; then
                    ISSUES="$ISSUES  ⚠️ $f: 发现敏感信息模式\n"
                    break
                fi
            done
        fi
...
if [ -n "$UNCOMMITTED" ]; then
    ...
    if [ -n "$SAFE_FILES" ]; then
        # 安全检查通过后再提交
        if security_check "$SAFE_FILES"; then
